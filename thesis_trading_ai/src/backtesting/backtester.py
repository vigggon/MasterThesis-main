...
def generate_trade_table(
    ...
    for i in range(len(pred)):
        ...
        entry_date = pd.to_datetime(df_raw.loc[idx_in_raw, "datetime"]).date()
        future_candidates = df_raw.iloc[idx_in_raw + 1 : idx_in_raw + 1 + MAX_HOLD_CANDLES]
        future = future_candidates[pd.to_datetime(future_candidates["datetime"]).dt.date == entry_date]
        ...
        # Create trade record (Standardized Format)
        return_pct = (raw_pnl_pt / entry_price) * 100.0 if entry_price > 0 else 0.0
        trade_data = {
            "timestamp_open": bar_time,
            "timestamp_close": close_time,
            "position": "long" if direction == 1 else "short",
            "entry_price": round(entry_price, 2),
            "exit_price": round(exit_price, 2),
            "pnl": round(raw_pnl_pt, 2),
...
def run_backtester(
    ...
    returns, times, pred, y, probs, atr_values, X, feature_cols = load_backtest_data_and_predictions(
    ...
    # Calculate realistic return based on professional risk standards (0.5% - 0.75% per R)
    total_return_units = float(returns.sum())
    realistic_return_low = (total_return_units * risk_pct)  # Standard Professional (e.g. 0.5%)
    realistic_return_high = (total_return_units * (risk_pct * 1.5)) # Aggressive (e.g. 0.75%)
    
    # CVaR 95 (Expected Shortfall) of R-multiples
    sorted_rets = np.sort(returns)
    cutoff_idx = int(0.05 * len(sorted_rets))
    cvar95 = np.mean(sorted_rets[:cutoff_idx]) if cutoff_idx > 0 else (np.min(returns) if len(returns) > 0 else 0)
    ...
    df_trades = generate_trade_table(
        ...
        X_features=X.numpy() if hasattr(X, 'numpy') else X,
    ...
    file_prefix = "bilstm" if model_name == "lstm" else model_name
    df_trades.to_csv(out_dir / f"{file_prefix}_trades.csv", index=False)

    # Calculate correct drawdown series using account percentage (base 1.0)
    peak_pct = np.maximum.accumulate(equity_pct)
    drawdown_pct = (equity_pct - peak_pct) / np.where(peak_pct > 0, peak_pct, 1)
    ...
    df_equity.to_csv(out_dir / f"{file_prefix}_equity.csv", index=False)
    pd.DataFrame([metrics]).to_csv(out_dir / f"{file_prefix}_metrics.csv", index=False)
